...
rule hmmsearch_env:
    ...
    log:
        "logs/hmmsearch/{gene}/{batch}.log"
    benchmark:
        "benchmarks/hmmsearch/{gene}/{batch}.benchmark.txt"
    threads:
...
rule merge_hmmsearch_env_headers:
    input:
        expand(fmt_hmm_hitnames, gene=config['genes_custom'], batch=BATCHES),
    output:
...
rule get_hmms_best_hit:
    input:
        expand(fmt_table_hmm, gene=GENES_TREE, batch=BATCHES),
    output:
        fn_hmms_best_hit,
    log:
