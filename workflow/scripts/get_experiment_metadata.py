...
def parse_sample_info(fn_sample_info, cfg, experiment):
    ...
    if fn_sample_info is None: # Did not download reads from NCBI
        ...
        if dict_meta is None:
            raise ValueError(f"Metadata not defined for experiment {experiment}")
        return 
    else: 
        dict_meta = {}
        regex = dict_config['regex_contition']  # condition string matching 
        ...
        if dict_config['method_counts'] == 'salmon-srr':  # Get info from the srr_info table
            ...
            for sam, info in df_info[['Run',col]].values:
                ...
                sam_col = 'count_' + sam
                dict_meta[sam_col] = {'condition':cond}
...
def main(
    ...
    dict_sample_info = parse_sample_info(sample_info, cfg, experiment)
    out_table = pd.DataFrame(dict_sample_info)
